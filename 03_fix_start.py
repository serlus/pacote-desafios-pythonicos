...
def fix_start(s):
    # +++ SUA SOLUÇÃO 1ª +++
    first_l = s[0]
    s_div = s[1:].split(first_l)
    s_join = first_l + '*'.join(s_div)
    return s_join
...
if __name__ == '__main__':
    ...
    test(fix_start_2, 'babble', 'ba**le')
    test(fix_start_2, 'aardvark', 'a*rdv*rk')
    test(fix_start_2, 'google', 'goo*le')
    test(fix_start_2, 'donut', 'donut')
